...
function brief_received(brief_data) {
  ...
  current_weather_request =
    "weather?lat=" + parseInt(lat) + "&lon=" + parseInt(lng);
  ...
  for (i = 0; i < 6; i++) {
    ...
    switch (i) {
      ...
      case 4:
        tag_label = "Baro";
        tag_value =
          Math.round(weather_data.main.pressure) +
          " mb (" +
          (Math.round(weather_data.main.pressure * 0.0295 * 100) / 100).toFixed(
            2
          ) +
          " in.)";
        break;
...
function set_world_time(timezone) {
  ...
  const delta =
    moment.tz.zone(moment.tz.guess()).utcOffset(Date.now()) / 60 -
    moment.tz.zone(timezone).utcOffset(Date.now()) / 60;
  ...
  world_time = setInterval(function () {
    ...
    if (country_dateEl) {
      country_dateEl.innerHTML = "<p>" + day + "</p>";
    }
    if (country_timeEl) {
      country_timeEl.innerHTML =
        "<p>" +
        time +
        "<span class='subscript'>" +
        part +
        "</span><span class='time_diff'> (" +
        time_diff +
        ")</span></p>";
    }
    if (country_timezoneEl) {
      country_timezoneEl.innerHTML = "<p> Timezone (" + timezone + ")</p>";
    }
  }, 1000);
}
function set_avg_temps(temp_data) {
  ...
  for (let i = 0; i < 12; i++) {
    const month_tag = create_tagbox(
      months[i].slice(0, 3),
      format_temp(temp_data.weather[months[i]].tAvg)
    );
...
function set_currency(currency_data) {
  ...
  const currency_name = create_tagbox(
    "Currency Used",
    currency_data.name + " (" + currency_data.code + ")"
  );
  const currency_rate = create_tagbox(
    "Rate to USD",
    currency.format(currency_data.rate) + " = $1.00"
  );
...
function set_other_info(brief_data) {
  ...
  // Drinking Water
  const drinking_water = brief_data.water.short
    ? brief_data.water.short.toUpperCase()
    : "No Data Provided";
  ...
  if (brief_data.vaccinations.length > 0) {
    brief_data.vaccinations.forEach(function (v) {
      other_info_wrapperEl.appendChild(
        create_tagbox("Vaccination - " + v.name, v.message)
      );
    });
  }
}
...
function format_temp(celsius) {
  ...
  const far_format =
    " (" + parseInt((Math.round(celsius) * 9) / 5 + 32) + "\u2109)";
...
countryNameEl.addEventListener("change", function (event) {
  ...
  if (brief_url) {
    ...
    alert(
      "No matching country has been selected! Please select a copy on the list"
    );
    countryNameEl.focus();
  }
});
